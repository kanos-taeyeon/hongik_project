...
import sys
import lief
from core import sub_PEFeatureExtractor
import tqdm
import jsonlines
import pandas as pd
...
class sub_Extractor:
    ...
    def extract_features(self, sample):
        ...
        """
        sub_extractor = sub_PEFeatureExtractor(self.features)
        fullpath = os.path.join(os.path.join(self.datadir, sample))

        try:
            binary = open(fullpath, 'rb').read()
            sub_feature = sub_extractor.raw_features(binary)
            sub_feature.update({"sha256": sample}) # sample name(hash)
            sub_feature.update({"label" : self.data[self.data.hash==sample].values[0][1]}) #label
