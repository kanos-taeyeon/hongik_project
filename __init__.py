...
from .sub_features import sub_PEFeatureExtractor
...
def vectorize(irow, raw_features_string, X_path, y_path, nrows, features, dim):
    ...
    y = np.memmap(y_path, dtype=np.float64, mode="r+", shape=nrows)
    y[irow] = raw_features["label"]

    X = np.memmap(X_path, dtype=np.float64, mode="r+", shape=(nrows, dim))
    X[irow] = feature_vector

def sub_vectorize(irow, raw_features_string, X_path, y_path, nrows, features, dim):
    """
    Vectorize a single sample of raw features and write to a large numpy file
    """
    sub_extractor = sub_PEFeatureExtractor(features, dim)
    raw_features = json.loads(raw_features_string)
    sub_feature_vector = sub_extractor.process_raw_features(raw_features)

    y = np.memmap(y_path, dtype=np.float64, mode="r+", shape=nrows)
    y[irow] = raw_features["label"]

    X = np.memmap(X_path, dtype=np.float64, mode="r+", shape=(nrows, dim))
    X[irow] = sub_feature_vector
...
def sub_vectorize_unpack(args):
    """
    Pass through function for unpacking vectorize arguments
    """
    return sub_vectorize(*args)
...
def vectorize_subset(X_path, y_path, raw_feature_paths, nrows, features, dim):
    ...
    X = np.memmap(X_path, dtype=np.float64, mode="w+", shape=(nrows, dim))
    y = np.memmap(y_path, dtype=np.float64, mode="w+", shape=nrows)
    del X, y
...
def sub_vectorize_subset(X_path, y_path, raw_feature_paths, nrows, features, dim):
    """
    Vectorize a subset of data and write it to disk
    """
    # Create space on disk to write features to
    ...
    argument_iterator = ((irow, raw_features_string, X_path, y_path, nrows, features, dim)
                         for irow, raw_features_string in enumerate(raw_feature_iterator(raw_feature_paths)))
    for _ in tqdm.tqdm(pool.imap_unordered(sub_vectorize_unpack, argument_iterator), total=nrows):
        pass

def create_vectorized_features_pre(jsonlpath, data_dir, rows, features, dim):
    print("Vectorizing Dataset set")
    X_path = os.path.join(data_dir, "X_pre.dat")
    y_path = os.path.join(data_dir, "y_pre.dat")
    raw_feature_paths = [jsonlpath]
    vectorize_subset(X_path, y_path, raw_feature_paths, rows, features, dim)

def read_vectorized_features_pre(data_dir, rows, dim):
    X_path = os.path.join(data_dir, "X_pre.dat")
    y_path = os.path.join(data_dir, "y_pre.dat")
    X = np.memmap(X_path, dtype=np.float64, mode="r", shape=(rows, dim))
    y = np.memmap(y_path, dtype=np.float64, mode="r", shape=rows)
...
def sub_create_vectorized_features(jsonlpath, data_dir, rows, features, dim):
    print("sub_Vectorizing Dataset set")
    X_path = os.path.join(data_dir, "X.dat")
    y_path = os.path.join(data_dir, "y.dat")
    raw_feature_paths = [jsonlpath]
    sub_vectorize_subset(X_path, y_path, raw_feature_paths, rows, features, dim)
...
def predict_sample(main_model, sub_model, file_data, featurelist,sub_featurelist):
    """
    Predict a PE file with an LightGBM model
    """
    if lief.PE.is_pe(list(file_data)):
        extractor = PEFeatureExtractor(featurelist)
        features = np.array(extractor.feature_vector(file_data), dtype=np.float64)
        result = main_model.predict([features])[-1]
    else:
        sub_extractor = sub_PEFeatureExtractor(sub_featurelist)
        sub_features = np.array(sub_extractor.feature_vector(file_data), dtype=np.float64)
        result = sub_model.predict([sub_features])[-1]

    return result

def numpy_sample(file_data, featurelist):
